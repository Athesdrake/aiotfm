...
class RoomList:
	...
	def __init__(
		self, gamemode: GameMode, rooms: List[RoomEntry], pinned_rooms: List[RoomEntry], gamemodes: List[GameMode]
	):
			...
			if is_pinned:
				...
				if command == "lm":
					...
					for mode in args.split("&~"):
						...
						entries.append(
							RoomEntry(name, room.language, room.country, int(count), command="mjj", args="m " + name)
						)

					pinned.append(room)
				else:
					pinned.append(
						RoomEntry(name, language, country, player_count, command=command, args=args, is_pinned=True)
					)
			else:
				...
				rooms.append(RoomEntry(name, language, country, player_count, **kwargs))
